-- ============================================================
-- 0022 Knockout matches scored like the group stage (June 28, 2026)
-- Applied live via Supabase MCP as: 0022_knockout_scored_like_group_6_2_0
--
-- Owner clarification: knockout matches are scored the SAME as the group stage
-- — 6 exact / 2 correct result / 0 wrong — NOT a special bonus and NOT zero.
-- (0019 wrongly dropped knockout match scoring; this restores it as plain
-- 6/2/0 across ALL stages. The special KO bonus layer stays removed.)
-- Final leaderboard_view:
--   match_points    = 6/2/0 over ALL matches (group + knockout scorelines)
--   bonus_points    = bracket advancement (incl champion 55) + champion 50 + boot 25 + ball 25
--   knockout_points = knockout-stage 6/2/0 + bracket advancement (tiebreaker)
-- exact/result/wrong counts span all matches. Column order/types unchanged.
-- ============================================================

    ...
    <div className="pb-24 max-w-2xl mx-auto">
      ...
      {honors.map((h) => (
        <section key={h.tournament.id} className="mt-6 overflow-hidden rounded-2xl border border-amber-500/30 bg-[#0C111C]">
          ...
          >
            <div className="text-[11px] font-semibold uppercase tracking-[.35em] text-amber-200/80">
              {es ? h.tournament.nameEs : h.tournament.nameEn}
              {h.leagueName ? ` · ${h.leagueName}` : ''}
            </div>
            <div className="mt-1 text-[10px] uppercase tracking-[.25em] text-muted-foreground">
              ...
              {[1, 0, 2].map((i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 truncate text-[12px] font-bold">{p.displayName}</div>
                    <div className="text-[11px] text-muted-foreground tabular-nums">{p.points} pts</div>
                  </div>
                );
              })}
            </div>
          ...
          <div className="border-t border-border/40">
            ...
            {(h.standings ?? []).map((r) => (
              <div
                key={r.userId}
                className={cn(
                  'flex items-center gap-3 border-t border-border/25 px-4 py-2 text-[13px]',

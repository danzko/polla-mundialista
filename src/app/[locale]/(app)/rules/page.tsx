...
export default async function RulesPage({ params }: RulesPageProps) {
  ...
  return (
    <div className="pb-24 max-w-2xl mx-auto">
      ...
      {/* SEASON MECHANICS (league phase) */}
      {ucl && (
        <section className="mt-4 rounded-2xl border border-amber-500/40 bg-amber-500/[0.06] p-4">
          <h2 className="font-bold flex items-center gap-2">⚡ {es ? 'Mecánicas de temporada' : 'Season mechanics'}</h2>
          <ul className="mt-3 space-y-2.5 text-[13px]">
            {[
              { ic: '⭐', t: es ? 'La Fija' : 'The Banker', d: es ? 'Cada jornada marcas UN partido como tu Fija: sus puntos cuentan doble (12 exacto / 4 resultado / 0). Puedes moverla hasta que ese partido cierre.' : 'Each matchday you mark ONE game as your banker: its points count double (12 exact / 4 result / 0). Movable until that game locks.' },
